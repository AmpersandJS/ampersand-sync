          ...
          // XHR does not return error message >= 400.
            ...
            if ( options.error ) {
                 if ( resp.statusCode >= 400 && resp.statusCode < 500 ) {
                     return options.error(resp, 'error', '[ampersand-sync] HTTP Status Code 4xx: Client Error ' + resp.statusCode );
                 }
                 else if ( resp.statusCode >= 500 && resp.statusCode < 600 ) {
                     return options.error(resp, 'error', '[ampersand-sync] HTTP Status Code 5xx: Server Error ' + resp.statusCode );
                 }
                 else {
                     return options.error(resp, 'error', '[ampersand-sync] HTTP Status Code ' + resp.statusCode );
                 }
             }
          }
